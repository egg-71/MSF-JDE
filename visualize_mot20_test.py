...
def visualize_mot20_test(model_path, test_img_dir, output_dir, tracker_yaml="yolojdetracker.yaml",
                         save_video=True, save_images=False, max_frames=None):
    """
    可视化MOT20测试集（平铺格式）

    Args:
        test_img_dir: 测试集图片目录，例如 /root/autodl-tmp/YOLOTrack_MOT20/images/test
    """
    print("=" * 60)
    print("🎬 MOT20测试集可视化")
    print("=" * 60)
    ...
    # 按序列分组（文件名格式: MOT20-04_000001.jpg）
    sequences = {}
    ...
    for seq in sorted(sequences.keys()):
        print(f"  - {seq}: {len(sequences[seq])} 帧")

    # 创建输出目录
    vis_dir = Path(output_dir) / "MOT20_test_visualization"
    ...
    for seq_name in sorted(sequences.keys()):
        ...
        imgs = sorted(sequences[seq_name])
...
if __name__ == "__main__":
    ...
    TEST_IMG_DIR = "/root/autodl-tmp/YOLOTrack_MOT20/images/test"
    OUTPUT_DIR = "reid_xps/CH-jde-msf-b8-e100-sz1280-20260313-190642"

    # 运行可视化
    visualize_mot20_test(
