...
if len(sequence_data_list) > 0:
    ...
    print(f"帧数: {len(imgs)}")
    print(f"平均每帧检测数: {len(sequence_data) / len(imgs):.1f}")
    ...
    if os.path.exists(backup_path):
        old_lines = sum(1 for line in open(backup_path))
        print(f"\n📊 对比:")
        print(f"  旧结果: {old_lines} 行")
        print(f"  新结果: {len(sequence_data)} 行")
        print(f"  改进: {len(sequence_data) - old_lines:+d} 行 ({(len(sequence_data) / old_lines - 1) * 100:+.1f}%)")
else:
    print("\n❌ 没有检测到任何目标！")
